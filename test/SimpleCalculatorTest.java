...
public class SimpleCalculatorTest {
    ...
    @Test
    @DisplayName("Testing div 1 / 0")
    public void testDiv1and0(){
        int first = 1;
        int second = 0;
        int expected = 0;

        int results = SimpleCalculator.div(first, second);
        assertNotEquals(expected, results);
    }

    @Test
    @DisplayName("Testing div 0 / 1")
    public void testDiv0And1(){
        int first = 0;
        int second = 1;
        int expected = 0;

        int result = SimpleCalculator.div(first, second);
        assertEquals(expected, result);
    }
}
